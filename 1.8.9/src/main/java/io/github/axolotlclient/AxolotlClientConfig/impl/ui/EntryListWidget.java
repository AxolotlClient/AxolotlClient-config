...
public abstract class EntryListWidget<E extends EntryListWidget.Entry<E>> extends AbstractParentElement implements Drawable, Selectable {
	...
	protected void renderScrollbar(int x, int y, int width, int height) {
		...
		bufferBuilder.vertex(x, this.bottom, 0.0).texture(0.0, 1.0).color(0, 0, 0, 255).next();
		...
		bufferBuilder.vertex(x, this.top, 0.0).texture(0.0, 0.0).color(0, 0, 0, 255).next();
		tessellator.draw();
		bufferBuilder.begin(7, VertexFormats.POSITION_TEXTURE_COLOR);
		bufferBuilder.vertex(x, y + height, 0.0).texture(0.0, 1.0).color(128, 128, 128, 255).next();
		bufferBuilder.vertex(l, y + height, 0.0).texture(1.0, 1.0).color(128, 128, 128, 255).next();
		bufferBuilder.vertex(l, y, 0.0).texture(1.0, 0.0).color(128, 128, 128, 255).next();
		bufferBuilder.vertex(x, y, 0.0).texture(0.0, 0.0).color(128, 128, 128, 255).next();
		tessellator.draw();
		bufferBuilder.begin(7, VertexFormats.POSITION_TEXTURE_COLOR);
		bufferBuilder.vertex(x, y + height - 1, 0.0).texture(0.0, 1.0).color(192, 192, 192, 255).next();
		bufferBuilder.vertex(l - 1, y + height - 1, 0.0).texture(1.0, 1.0).color(192, 192, 192, 255).next();
		bufferBuilder.vertex(l - 1, y, 0.0).texture(1.0, 0.0).color(192, 192, 192, 255).next();
		bufferBuilder.vertex(x, y, 0.0).texture(0.0, 0.0).color(192, 192, 192, 255).next();
		tessellator.draw();
				...
				if (entry != null) {
					if (entry.mouseClicked(mouseX, mouseY, button)) {
						...
						if (entry2 != entry && entry2 instanceof ParentElement) {
							((ParentElement) entry2).setFocusedChild(null);
						}
