	...
	public float drawStringWithFormatting(long ctx, NVGFont font, String text, float x, float y, Color color) {
		if (text.contains("§")) { // work out Minecraft's formatting codes
			...
			if (matcher.find()) {
				...
				NVGColor textColor = NVGColorHelper.toNVG(color);
				float partX = x;
				float lastPartX;
						...
						switch (first) {
							...
							default:
								if (MINECRAFT_COLOR_CODES.containsKey(first)) {
									textColor = NVGColorHelper.toNVG(MINECRAFT_COLOR_CODES.get(first).withAlpha(color.getAlpha()));
									part = part.substring(1);
								} else {
									textColor = NVGColorHelper.toNVG(color);
								}
						}
					}
		...
		NanoVG.nvgFillColor(ctx, NVGColorHelper.toNVG(color));
		font.bind();
