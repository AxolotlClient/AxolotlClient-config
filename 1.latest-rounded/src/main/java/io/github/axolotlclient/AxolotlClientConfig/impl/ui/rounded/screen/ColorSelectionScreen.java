...
import io.github.axolotlclient.AxolotlClientConfig.impl.ui.NVGColorHelper;
	...
	public void render(GuiGraphics graphics, int mouseX, int mouseY, float delta) {
		NVGUtil.wrap(ctx -> {
			...
			if (paint == null || paint.address() == 0) {
				int image = RoundedDrawUtil.nvgCreateImage(ctx, ResourceLocation.fromNamespaceAndPath("axolotlclientconfig", "textures/gui/colorwheel.png"));
			...
			nvgBeginPath(ctx);
			nvgRoundedRect(ctx, selectorX, selectorY, selectorRadius * 2, selectorRadius * 2, selectorRadius);
			nvgStrokeColor(ctx, NVGColorHelper.toNVG(Colors.BLACK));
