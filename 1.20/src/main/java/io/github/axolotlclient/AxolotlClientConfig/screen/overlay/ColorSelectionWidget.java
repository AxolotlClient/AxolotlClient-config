...
public class ColorSelectionWidget extends Overlay {
	...
	public void render(GuiGraphics graphics, int mouseX, int mouseY, float delta) {
		...
		graphics.drawText(MinecraftClient.getInstance().textRenderer, Text.translatable("currentColor").append(":"), currentRect.x, currentRect.y - 10, -1, true);

		DrawUtil.fillRect(graphics, currentRect, option.get());
		DrawUtil.outlineRect(graphics, currentRect, Color.DARK_GRAY.withAlpha(127));

		RenderSystem.setShaderColor(1, 1, 1, 1);

		graphics.drawTexture(wheel, pickerImage.x, pickerImage.y, 0, 0, pickerImage.width, pickerImage.height, pickerImage.width, pickerImage.height);
		DrawUtil.outlineRect(graphics, pickerOutline, Color.DARK_GRAY);
		super.postRender(graphics);
	}
