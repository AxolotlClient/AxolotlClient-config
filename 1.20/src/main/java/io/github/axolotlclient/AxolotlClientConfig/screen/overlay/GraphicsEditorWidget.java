	...
	public void render(GuiGraphics graphics, int mouseX, int mouseY, float delta) {
		...
		for (int x = 0; x < gridCollumns; x++) {
			for (int y = 0; y < gridRows; y++) {
				if (pixels[x][y] != 0) {
					graphics.fill(gridX + x * pixelSize, gridY + y * pixelSize, gridX + x * pixelSize + pixelSize, gridY + y * pixelSize + pixelSize, pixels[x][y]);
				} else {
		...
		DrawUtil.outlineRect(graphics, gridX - 1, gridY - 1, maxGridWidth + 2, maxGridHeight + 2, -1);
		...
		DrawUtil.outlineRect(graphics, gridX + lastChangedPixel[0] * pixelSize, gridY + lastChangedPixel[1] * pixelSize, pixelSize, pixelSize, Color.SELECTOR_GREEN.getAsInt());
