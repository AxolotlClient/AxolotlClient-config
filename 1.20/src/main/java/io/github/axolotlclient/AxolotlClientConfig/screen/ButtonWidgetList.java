...
public class ButtonWidgetList extends ElementListWidget<ButtonWidgetList.Pair> {
	...
	public class Pair extends ElementListWidget.Entry<Pair> {
		...
		public void render(GuiGraphics graphics, int index, int y, int x, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean hovered, float tickDelta) {
			...
			if (this.left != null) {
				this.left.setY(y);
				this.left.render(graphics, mouseX, mouseY, tickDelta);
			}

			if (this.right != null) {
				this.right.setY(y);
				this.right.render(graphics, mouseX, mouseY, tickDelta);
			}
			...
			if (AxolotlClientConfigConfig.showCategoryTooltips.get()) {
				if (super.left != null && super.left.isMouseOver(mouseX, mouseY)) {
					if (AxolotlClientConfigConfig.showQuickToggles.get() && ((CategoryWidget) super.left).enabledButton != null && ((CategoryWidget) super.left).enabledButton.isMouseOver(mouseX, mouseY)) {
						renderTooltip(graphics, ((CategoryWidget) super.left).enabledButton.option, mouseX, mouseY);
					} else {
						renderTooltip(graphics, left, mouseX, mouseY);
					}
					...
					if (AxolotlClientConfigConfig.showQuickToggles.get() && ((CategoryWidget) super.left).enabledButton != null && ((CategoryWidget) super.left).enabledButton.isFocused()) {
						renderTooltip(graphics, ((CategoryWidget) super.left).enabledButton.option, super.left.getX() + super.left.getWidth() / 2, super.left.getY());
					} else {
						renderTooltip(graphics, left, super.left.getX() + super.left.getWidth() / 2, super.left.getY());
					}
				}
				if (super.right != null && super.right.isMouseOver(mouseX, mouseY)) {
					if (AxolotlClientConfigConfig.showQuickToggles.get() && ((CategoryWidget) super.right).enabledButton != null && ((CategoryWidget) super.right).enabledButton.isMouseOver(mouseX, mouseY)) {
						renderTooltip(graphics, ((CategoryWidget) super.right).enabledButton.option, mouseX, mouseY);
					} else {
						renderTooltip(graphics, right, mouseX, mouseY);
					}
					...
					if (AxolotlClientConfigConfig.showQuickToggles.get() && ((CategoryWidget) super.right).enabledButton != null && ((CategoryWidget) super.right).enabledButton.isFocused()) {
						renderTooltip(graphics, ((CategoryWidget) super.right).enabledButton.option, super.right.getX() + super.right.getWidth() / 2, super.right.getY());
					} else {
						renderTooltip(graphics, right, super.right.getX() + super.right.getWidth() / 2, super.right.getY());
					}
				}
			}
		}
	...
	public class OptionEntry extends Pair {
		...
		public void render(GuiGraphics graphics, int index, int y, int x, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean hovered, float tickDelta) {
			...
			drawName(graphics, name, ButtonWidgetList.this.width / 2 - 125, y + 5, ButtonWidgetList.this.width / 2 - 10, y + 5 + client.textRenderer.fontHeight, -1);
			left.setY(y);
			left.render(graphics, mouseX, mouseY, tickDelta);

			renderX = ButtonWidgetList.this.width / 2 - 125;
		}

		private void drawName(GuiGraphics graphics, String text, int x, int y, int x2, int y2, int color) {
			...
			if (n > p) {
				...
				graphics.enableScissor(x, y, x2, y2);
				graphics.drawText(client.textRenderer, text, x - (int) g, o, color, true);
				graphics.disableScissor();
			} else {
				graphics.drawCenteredShadowedText(client.textRenderer, text, (x + x2) / 2, o, color);
			}
		}
		...
		public void renderTooltips(GuiGraphics graphics, int mouseX, int mouseY) {
			if (AxolotlClientConfigConfig.showOptionTooltips.get()) {
				if (mouseX >= renderX && mouseX <= (renderX + nameWidth) && mouseY >= left.getY() && mouseY <= left.getY() + left.getHeight() ||
					...
					renderTooltip(graphics, option, mouseX, mouseY);
				} else if (left.isFocused()) {
					renderTooltip(graphics, option, left.getX() + left.getWidth() / 2, left.getY());
				}
			}
		}
