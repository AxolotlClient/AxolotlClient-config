...
public class ColorSelectionScreen extends AbstractScreen {
	...
	public void render(long ctx, double mouseX, double mouseY, float delta) {
		DrawUtil.drawCenteredString(MatrixStackProvider.getInstance().getStack(), client.textRenderer, title, width/2, 20, Colors.WHITE.toInt(), true);

		MinecraftClient.getInstance().getTextureManager().bindTexture(texture);
		DrawableHelper.drawTexture(MatrixStackProvider.getInstance().getStack(),
			(int) selectorX, (int) selectorY, 0, 0, selectorRadius*2, selectorRadius*2, selectorRadius*2, selectorRadius*2);

		DrawUtil.outlineRect(MatrixStackProvider.getInstance().getStack(),
			(int) selectorX, (int) selectorY, selectorRadius*2, selectorRadius*2, Colors.BLACK.toInt());

		client.textRenderer.draw(MatrixStackProvider.getInstance().getStack(),
			I18n.translate("option.current"), buttonsX, 40, Colors.WHITE.toInt());

		DrawUtil.fillRect(MatrixStackProvider.getInstance().getStack(),
			buttonsX, 55, 150, 40, option.get().get().toInt());
		DrawUtil.outlineRect(MatrixStackProvider.getInstance().getStack(),
			buttonsX, 55, 150, 40, Colors.BLACK.toInt());

		client.textRenderer.draw(MatrixStackProvider.getInstance().getStack(),
			I18n.translate("option.chroma"), buttonsX, 105, Colors.WHITE.toInt());
		client.textRenderer.draw(MatrixStackProvider.getInstance().getStack(),
			I18n.translate("option.alpha"), buttonsX, 150, Colors.WHITE.toInt());
	}
