...
public class ColorSelectionScreen extends Screen {
	...
	public void init() {
		super.init();
		addDrawableChild(new ButtonWidget(width / 2 - 75, height - 40, 150, 20,
			ScreenTexts.BACK, buttonWidget -> closeScreen()));
	...
	public void render(MatrixStack graphics, int mouseX, int mouseY, float delta) {
		...
		int y = 105;
		drawTextWithShadow(graphics, client.textRenderer, Text.translatable("option.chroma"), buttonsX, y, Colors.WHITE.toInt());
		y += 45;
		if (height > 300) {
			drawTextWithShadow(graphics, client.textRenderer, Text.translatable("option.speed"), buttonsX, y, Colors.WHITE.toInt());
			y += 45;
		}
		drawTextWithShadow(graphics, client.textRenderer, Text.translatable("option.alpha"), buttonsX, y, Colors.WHITE.toInt());
	}
