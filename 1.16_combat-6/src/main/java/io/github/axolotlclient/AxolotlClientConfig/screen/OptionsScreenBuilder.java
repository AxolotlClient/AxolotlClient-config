...
public class OptionsScreenBuilder extends Screen {
	...
	public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        ...
        drawCenteredString(matrices, textRenderer, cat.getTranslatedName(), width/2, 25, -1);
    ...
    public void setOverlay(Overlay overlay){
        MinecraftClient.getInstance().openScreen(this.overlay = overlay);
    }
    ...
    public void init() {
        if(!pickerWasOpened) {
            ...
            this.addButton(backButton = new ButtonWidget(this.width / 2 - 100, this.height - 40, 200, 20, ScreenTexts.BACK, buttonWidget -> {
				if (isPickerOpen()) {
					closeOverlay();
				} else {
            ...
            this.addButton(searchWidget = new TextFieldWidget(MinecraftClient.getInstance().textRenderer, width - 120, 20, 100, 20, new TranslatableText("search")) {
                ...
                public boolean mouseClicked(double mouseX, double mouseY, int button) {
                    ...
                    setFocused(false);
                    setSuggestion(Formatting.ITALIC + new TranslatableText("search").append("...").getString());
                    return false;
                }

                @Override
                public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
                    if ((keyCode == GLFW.GLFW_KEY_SPACE || keyCode == GLFW.GLFW_KEY_ENTER) && cat.toString().toLowerCase(Locale.ROOT).contains(modid.toLowerCase(Locale.ROOT))) {
                        MinecraftClient.getInstance().openScreen(new OptionsScreenBuilder(OptionsScreenBuilder.this, getAllOptions(), modid));
                    }
                ...
                protected MutableText getNarrationMessage() {
                    return super.getNarrationMessage().append(new TranslatableText("narration.type_to_search"));
                }
            });
