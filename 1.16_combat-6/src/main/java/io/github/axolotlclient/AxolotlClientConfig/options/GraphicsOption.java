...
public class GraphicsOption extends OptionBase<int[][]> {
    ...
    protected CommandResponse onCommandExecution(String arg) {
        ...
        if (arg.length() > 0) {
            try {
                setValueFromJsonElement(new JsonParser().parse(arg));
    ...
    public void bindTexture() {
        ...
        for (int x = 0; x < width; x++) {
            for (int y = 0; y < height; y++) {
                int color = get()[x][y];
                image.setPixelColor(x, y, NativeImage.getAbgrColor(NativeImage.getAlpha(color), NativeImage.getRed(color), NativeImage.getGreen(color), NativeImage.getBlue(color)));
            }
        }
        texture.upload();
