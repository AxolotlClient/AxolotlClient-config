...
public class VanillaButtonWidget extends ButtonWidget {
	...
	protected void drawWidget(int mouseX, int mouseY, float delta) {
		...
		drawTexture(MatrixStackProvider.getInstance().getStack(),
			this.getX(), this.getY(), 0, 46 + k * 20, this.getWidth() / 2, this.getHeight());
		drawTexture(MatrixStackProvider.getInstance().getStack(),
			this.getX() + this.getWidth() / 2, this.getY(), 200 - this.getWidth() / 2, 46 + k * 20, this.getWidth() / 2, this.getHeight());
		int l = 14737632;
		if (!this.active) {
			l = 10526880;
